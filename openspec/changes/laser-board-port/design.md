...
- **The template crosses an architecture boundary; mirror shape, not the arch directory.** Verified against the LibreEcho-Linux-6.1 tree: the Gen-2 target builds ARM32 (`arch/arm/boot/dts/libreecho-radar-puffin.dts`, `arch/arm/configs/mt8163_arm32_defconfig`). MT8512 is a 64-bit Cortex-A53 SoC, so `laser` belongs under `arch/arm64/boot/dts/mediatek/` with an arm64 defconfig. Reuse the Gen-2 folder shape and the audio-contract method, but expect the DTS bindings, defconfig, and boot packaging to be arm64, not a copy of the arm32 files. This affects M1 (scaffold) and M2 (kernel to shell) more than a same-arch clone would.
